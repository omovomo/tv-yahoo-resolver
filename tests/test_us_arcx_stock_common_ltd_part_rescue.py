...
def row(prefix="AMEX", tv_type="stock", specs=("common",), isin="US2921022099", symbol="OGCP", currency="USD"):
...
def identity(figi="F1", share="SC1", st="Ltd Part", st2="Partnership Shares", ticker="OGCP"):
    return OpenFigiIdentity(figi, "C1", share, ticker, "Equity", st, st2, "US")
def candidate(symbol="OGCP", qt="EQUITY"):
    return YahooSearchCandidate(symbol, "PCX", qt, symbol, symbol)
def quote(symbol="OGCP", exchange="PCX", currency="USD", qt="EQUITY"):
    return YahooQuote(symbol, exchange, "NYSE Arca" if exchange == "PCX" else "NYSE", currency, qt, "us_market", symbol, symbol, 1.0, 0)
def resolver(ids=None, candidates=None, q=None):
    ...
    class OF:
        def map_jobs(self,jobs):
            assert all(j.get("micCode")=="ARCX" for j in jobs); return [ids for _ in jobs]
    class Y:
...
def run(r=None,res=None,b=None):
    ...
    return BatchResolver._us_arcx_stock_common_ltd_part_rescue(res,[r],[b]),res

def test_rescues_strict_audited_arcx_ltd_part_contract():
    out,res=run(); assert out[0].status=="VERIFIED"; assert out[0].mapping_method=="US_ARCX_STOCK_COMMON_FINNHUB_LTD_PART_EXACT_ISIN"; assert out[0].source_mic==out[0].target_mic=="ARCX"; assert res.stats["us_arcx_stock_common_ltd_part_rescue_matches"]==1
@pytest.mark.parametrize("r",[row(prefix="OTC"),row(prefix="NYSE"),row(tv_type="fund"),row(specs=("preferred",)),row(isin=None)])
def test_requires_amex_stock_common_exact_isin(r):
...
def test_requires_unique_exact_yahoo_arcx_usd_equity(candidates,q):
    assert run(res=resolver(candidates=candidates,q=q))[0][0].status=="REJECTED"
def test_policy422_reuses_policy421_verified_cache():
    from tv_market_identity.resolver import CACHE_COMPATIBLE_VERIFIED_RESOLVER_VERSIONS
    assert "0.4.21-policy421" in CACHE_COMPATIBLE_VERIFIED_RESOLVER_VERSIONS
