...
def row(prefix="OTC", tv_type="stock", specs=("common",), isin="US89619H1086", symbol="TRLC", currency="USD"):
    return TvRow(f"{prefix}:{symbol}", prefix, symbol, symbol, currency, tv_type, specs, None, None, None, isin)
def binding(r, reason="FINNHUB_TYPE_MISMATCH:Closed-End Fund"):
    return Binding(r.tv_id, r.symbol, r.prefix, r.currency, r.tv_type, "REJECTED", finnhub_type="Closed-End Fund", rejection_reason=reason)
def identity(figi="F1", share="SC1", st="Closed-End Fund", st2="Mutual Fund", ticker="TRLC"):
    return OpenFigiIdentity(figi, "C1", share, ticker, "Fund", st, st2, "US")
def candidate(symbol="TRLC", qt="EQUITY"):
    return YahooSearchCandidate(symbol, "PNK", qt, symbol, symbol)
def quote(symbol="TRLC", exchange="PNK", currency="USD", qt="EQUITY"):
...
def run(r=None,res=None,b=None):
    ...
    return BatchResolver._us_ootc_stock_common_closed_end_fund_rescue(res,[r],[b]),res

def test_rescues_strict_audited_ootc_closed_end_fund_contract():
    out,res=run(); assert out[0].status=="VERIFIED"; assert out[0].mapping_method=="US_OOTC_STOCK_COMMON_FINNHUB_CLOSED_END_FUND_EXACT_ISIN"; assert out[0].source_mic==out[0].target_mic=="OOTC"; assert res.stats["us_ootc_stock_common_closed_end_fund_rescue_matches"]==1
@pytest.mark.parametrize("r",[row(prefix="NASDAQ"),row(tv_type="fund"),row(specs=("preferred",)),row(isin=None)])
def test_requires_ootc_stock_common_exact_isin(r):
    assert run(r=r)[0][0].status=="REJECTED"
def test_requires_exact_rejection_reason():
    r=row(); assert run(r=r,b=binding(r,"FINNHUB_TYPE_MISMATCH:Ltd Part"))[0][0].status=="REJECTED"
...
def test_requires_unique_scoped_cef_mutualfund_exact_ticker_shareclass(ids):
...
def test_policy421_reuses_policy420_verified_cache():
    from tv_market_identity.resolver import CACHE_COMPATIBLE_VERIFIED_RESOLVER_VERSIONS
    assert "0.4.20-policy420" in CACHE_COMPATIBLE_VERIFIED_RESOLVER_VERSIONS
